...
def _resolver_directorio_datos():
    ruta_env = os.environ.get("BUSCADOR_NOTICIAS_DATA_DIR", "").strip()
    if ruta_env:
        return os.path.abspath(ruta_env)

    proyecto_actual = os.path.dirname(BASE_APP_DIR)
    proyecto_ambicioso = os.path.join(proyecto_actual, "Proyecto Ambicioso SEO")
    if os.path.isdir(proyecto_ambicioso):
        return os.path.join(proyecto_ambicioso, "db", "busqueda_noticias_shared")
...
OLLAMA_SIMILITUD_CACHE_PATH = os.path.join(DATA_APP_DIR, "ollama_similitud_cache.json")
...
if DATA_APP_DIR != BASE_APP_DIR:
    log.info(f"Directorio de datos compartido: {DATA_APP_DIR}")
...
FUENTES_RSS = [
    ...
    {"nombre": "Google News - Tendencias Globales",
     "url": "https://news.google.com/rss/search?q=viral+tendencia+colombia&hl=es-419&gl=CO&ceid=CO:es-419",
     "categorias": ["tendencias"], "tipo": "mundo"},
